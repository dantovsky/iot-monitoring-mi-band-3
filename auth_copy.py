...
class MiBand3(Peripheral):
    ...
    def get_steps(self):
        ...
        # why only 1 byte??
        # callories = struct.unpack('b', a[9])[0] if len(a) >= 10 else None # estoura nos unpack (ta a espera de Byte e retorna int)
        callories = a[9] if len(a) >= 3 else None # (by Loja)
        return {
            "steps": steps,
            "meters": meters,
    ...
    ### Lê o coracaozinho bonito (by Loja)
